 ...
 */
export const initSolutionViewGroup = (
	entity: Entity,
	solution: TPoint[],
) => {
	...
	if (solution) {
		...
		for (let i = 0; i < solution.length; i++) {
			...
			visibilityGraphComponent.solutionViewGroup.push(nodeView);
		...
		for (let i = 0; i < nodePairs.length; i++) {
			...
			visibilityGraphComponent.solutionViewGroup.push(nodeLinkView);
