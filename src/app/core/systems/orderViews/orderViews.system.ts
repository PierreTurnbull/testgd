...
import { ASortableView } from "../../../common/archetypes/sortableView/sortableView.archetype";
...
export const orderViews = () => {
	...
	orderedViewEntities.forEach((orderedViewEntity, key) => {
		...
		if (orderedViewEntity.hasComponent(CCenterView) && configManager.config.debug.showsEntityCenter) {
		...
		if (orderedViewEntity.hasRelation("hitboxes")) {
			const hitboxEntities = orderedViewEntity.getRelatedEntities("hitboxes");

			hitboxEntities.forEach(hitboxEntity => {
				if (hitboxEntity.hasComponent(CHitboxView) && configManager.config.debug.showsEntityHitbox) {
